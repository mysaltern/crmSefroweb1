...
use common\models\ImageGallerySearch;
...
class ImagegalleryController extends Controller
{
        ...
        if ($request->isAjax) {
            ...
            */
            ...
            if ($request->isGet) {
                ...
                if ($model->name = UploadedFile::getInstance($model, 'name')) {

                    $imgname = $model->name->baseName . time() . "." . $model->name->extension;
                    $model->name = UploadedFile::getInstance($model, 'name');
                    $model->name->saveAs('../../frontend/upload/imagegallery/' . $model->name->baseName . time() . "." . $model->name->extension);
                    $model->name = $imgname;
                    $model->save(false);
                }
                ...
                return [
                    ...
                    'content' => '<span class="text-success">تصویر با موفقیت ثبت شد</span>',
                    'footer' => Html::button('Close', ['class' => 'btn btn-default pull-left', 'data-dismiss' => "modal"]) .
                        Html::a('Create More', ['create'], ['class' => 'btn btn-primary', 'role' => 'modal-remote'])
                ];
            } else {
                return [
                    'title' => "Create new ImageGallery",
                    'content' => $this->renderAjax('create', [
                        'model' => $model,
                    ]),
                    'footer' => Html::button('Close', ['class' => 'btn btn-default pull-left', 'data-dismiss' => "modal"]) .
            ...
            */
            ...
            if ($request->isGet) {
                ...
                if ($model->name = UploadedFile::getInstance($model, 'name')) {
                    ...
                    $model->name = UploadedFile::getInstance($model, 'name');
                    $model->name->saveAs('../../frontend/upload/imagegallery/' . $model->name->baseName . "." . $model->name->extension);
                    $model->save(false);
                } else {
                    $model->name = $imgname;
                }
    ...
    /**
     * Delete an existing ImageGallery model.
     * For ajax request will return json object
    ...
    /**
     * Delete multiple existing ImageGallery model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
